...
const forecast = (lat, long, callback) => {
    ...
    request({ url, json: true }, (error, { body }) => {
        if (error) {
            callback('Cannot reach weather service', undefined, undefined)
        } else if (body.error) {
            callback('Unable to find location, try again', undefined, undefined)
        } else {
            ...
            The humidity is ${body.currently.humidity * 100}`, `The daily forecast: ${body.daily.summary}`)
        }
    })
}
